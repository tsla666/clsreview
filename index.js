...
async function sendMorningMessage() {
  try {
    ...
    const morningContent = `Stan 终极三问\n\n在完成复盘分析后 ，必须强制通过这三个关卡，否则不准执行计划：\n\n1 . 【背离校验】\n如果分析出的‘受力逻辑’是正确的，为什么目前的‘价格行为’表现得像个输家？\nStan 决策： 承认模型可能"领先"于市场，但身体要先撤出。不要用金钱去赌模型比市场更聪明。\n\n否: 价格背离逻辑 --> [缩减规模/保持观察] \n是: 价格确认逻辑 --> {Stan 2: 零基自问?}\n\n2. 【零基仓位测试】\n"假设我今天没有任何持仓（空仓），面对现在的价格和消息，我会一次性买入目前这么重的仓位吗？"\n\n现在价格不想重仓 --> H[减仓至舒适区] \n现在价格值得重仓 --> I[执行计划: 加码/持有]\n\n应用： 设定成长/低波攻守比值至少3比7\nStan 决策： 如果答案是"不会，我只敢买 2 成试探"，那么多出来的 5 成就是"非计划性头寸"，必须立刻抹掉\n\n3. 【资本守卫心态】\n"我现在持有的仓位，是否正在损耗我下一次‘扣动重仓扳机’的本金和心态？"\nStan 决策： 如果目前的波动让你无法冷静思考、开始报复性加仓，说明你已经失去了"职业猎人"的身份。减仓到你不再焦虑为止，哪怕逻辑依然看好。\n\n焦虑/想回本 --> [强行平仓/休息] \n冷静/专注 --> [终极判定: 目标象限]`;
...
function setupSchedule() {
  ...
  // 每天早上5点执行Stan决策辅助推送
    const job2 = schedule.scheduleJob('0 5 * * *', async function() {
      console.log('定时任务触发，开始发送Stan决策辅助消息...');
      await sendMorningMessage();
    });
    console.log('定时任务2设置成功，将在每天早上5:00执行');
